...
export default function TarjetaVencimiento({ item, onView, onSend, onWhatsApp }: { item: Vencimiento; onView?: ()=>void; onSend?: ()=>void; onWhatsApp?: ()=>void }){
    ...
    <Card>
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="text-sm mt-2">{item.fecha} • <span className={`px-2 py-0.5 rounded text-slate-900 ${color}`}> {item.criticidad}</span></div>
        </div>
        <div className="flex flex-col gap-2">
          ...
          <button onClick={onWhatsApp} className="text-sm text-green-600 dark:text-green-400 flex items-center gap-1"><MessageCircle size={14}/> WhatsApp</button>
        </div>
      </div>
    </Card>
  )
