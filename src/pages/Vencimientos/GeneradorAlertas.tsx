...
export default function GeneradorAlertas({ onClose, initialCliente }: { onClose: ()=>void; initialCliente?: string }){
  ...
  <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded w-96 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700">
        ...
        <div className="mb-2">
          ...
          <select value={canal} onChange={(e)=>setCanal(e.target.value as any)} className="block w-full border px-2 py-1 rounded bg-slate-100 dark:bg-slate-700 dark:text-slate-100">
        ...
        <div>
          ...
          <textarea value={mensaje} onChange={(e)=>setMensaje(e.target.value)} className="block w-full border px-2 py-1 rounded bg-slate-100 dark:bg-slate-700 dark:text-slate-100" rows={5} placeholder="Hola {cliente}, te recordamos que..." />
        </div>
